...
// ── Hero: ReactBits-style stack (hover → send top card to back) ──

function HeroImageSection() {
  const [stack, setStack] = useState(polaroids); // last item = top/front
  ...
  const sendTopToBack = () => {
    ...
    setStack((prev) => {
      ...
      return [top, ...next.slice(0, next.length - 1)];
    });
    setTimeout(() => animating[1](false), 500);
  };
  ...
  return (
    <section className="relative overflow-hidden pt-8 pb-0">
      ...
      <div className="container mx-auto max-w-5xl px-4">
        ...
        {/* Two-column hero: Polaroid left, Bio right */}
        <div className="relative mx-auto grid max-w-5xl grid-cols-1 items-start gap-8 md:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)] lg:gap-12">
          {/* Left: Polaroid stack — ReactBits-style, hover sends top card to back */}
          <motion.div
            initial={{ opacity: 0, x: -30 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.7, delay: 0.2 }}
            className="relative mx-auto w-full max-w-75 md:mx-0"
          >
            {/* Doodle arrow pointing at the stack */}
            <svg
              width="102"
              height="28"
              viewBox="0 0 102 28"
            ...
            {/*
              ReactBits Stack pattern (hover = send top card to back):
              - stack[] last item = front/top card
              - rotateZ fans from bottom-right (transformOrigin 90% 90%)
              - scale makes front card largest
              - onMouseEnter triggers sendTopToBack
            */}
            <div
              className="relative cursor-pointer"
              style={{ width: 340, height: 470 }}
              onMouseEnter={sendTopToBack}
            >
              {stack.map((card, index) => {
                ...
                return (
                  <motion.div
                    key={card.id}
                    className="bg-muted absolute inset-0 rounded-sm p-3 pb-14 shadow-xl"
                    style={{ zIndex: index }}
                    animate={{
                      rotateZ: (total - index - 1) * 6,
                      scale: 1 + index * 0.06 - total * 0.06,
                  ...
                  >
                    <div className="relative h-full w-full overflow-hidden rounded-xs">
                      <Image
                        ...
                        sizes="300px"
                        priority={card.id === 'portrait'}
                      />
                    </div>
                    {card.signature && (
                      <p
                        className="text-foreground mt-2 text-center text-xl"
          ...
          >
            ...
            <div className="mt-6 space-y-4">
              <p className="text-muted-foreground text-base leading-relaxed lg:text-lg">
                I am a full-stack developer and designer, specializing in the
                MERN stack and Next.js. My educational background includes a
